...
TAC_filter <- TAC %>%
  ...
  filter(., YEAR.y!= '2021') %>%
  filter(., YEAR.x > 2000)%>%
  filter(., HAULING_DEPTH > 400)


TAC_filter <- TAC %>%
  filter(Fullname == "OCTO_VUL")
...
10-50, 51-100, 101-200, and 201-500m
...
TAC_minmax = TAC_minmax %>% 
  mutate(ww_c= scale(ww_g, center = T, scale = F),
         temp_c = scale(BOTTOM_TEMPERATURE_BEGINNING, center = T, scale = F),
...
fit_TAC = brm(ww_g| vreal(num_st2, xmin, xmax) ~ fish_c*thetao_c + (1 | Site) + (1| HAUL_ID) + (1| YEAR.x),
              data = TAC_minmax,
...
depth_c = quantile(fit_TAC$data$depth_c, probs = c(0.25, 0.5, 0.75)) %>% 
  as_tibble() %>% rename(depth_c = value)

int_plot = conditional_effects(fit_TAC, effects = "fish_c", conditions = temp_c)

int_plot

int_plot = conditional_effects(fit_TAC, effects = "fish_c:temp_c")
...
int_plot_fish + labs(title = "Fishing effort effect on Merluzzo merluzzo", x = "Fishing effort", y = "λ") +
  theme_bw()
